...
export const Offers = () => {
    ...
    let winesCopy = useSelector((state)=> state.winesCopy)
    ...
    let descuentos = winesCopy?.result?.filter(e=> e.discount !== 0);
        ...
        : <div className={style.mainContainer}>
            ...
            <div className={style.containerCards}>
                {descuentos?.length !== 0   && wines_paginates.map(wine =>(
                    ...
                    <div key={wine._id}>
                        <CardProduct stock={wine.stock} id={wine._id} name={wine.name} year={wine.year} strain={wine.strain} producer={wine.producer} country={wine.producer} discount={wine.discount} price={Math.round(wine.price * (100 - wine.discount) / 100)} img={wine.img} category={wine.category.name} description={wine.description}/>
                    </div>
                ))} 
            </div> 
