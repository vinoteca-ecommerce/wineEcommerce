...
import { setLocalStorage, addLocalStorage, subLocalStorage, deleteLocalStorage, setShoppingCar, getShoppingCar, getWinesById, getWines, updateCart, getWinesCopy, updateCartSub, filterCart, deleteCart } from '../../redux/actions/actions';
...
function handleClickThrash(id){
    ...
    var xfilter = x.filter(e => e.wineActual._id !== id)
        ...
        dispatch(filterCart(id))// modifico el carrito

        dispatch(setShoppingCar(xfilter))// setear
                    ...
                    {cart?.map((st)=>(
                        <tbody key={st.wineActual._id} className={style.tableBody}>
                            <tr>
                                ...
                                <td>
                                 {<div><Button  onClick={()=>handleClickSub(st.wineActual._id)} style={{maxWidth: '30px', maxHeight: '30px', minWidth: '30px', minHeight: '30px',color:'#7f0000'}}><RemoveIcon/></Button>
                                    <p style={{display:'inline',color:'#7f0000',padding:'.2em .5em',margin:'.5em',border:'2px solid #7f0000', borderRadius:'1em', fontSize:'1em'}}>{st.cant}</p>
                                    <Button   onClick={()=>handleClickAdd(st.wineActual._id)} style={{maxWidth: '30px', maxHeight: '30px', minWidth: '30px', minHeight: '30px',color:'#7f0000'}}><AddIcon/></Button>
                                </div>}
                                </td>
